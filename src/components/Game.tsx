...
export default function Game({ onGameOver, onQuit, isPaused }: GameProps) {
  ...
  useEffect(() => {
    ...
    // Power-up Images
    const puShieldImg = new Image();
    puShieldImg.src = 'assets/powerup-shield.png';
    const puGunImg = new Image();
    puGunImg.src = 'assets/powerup-gun.png';
    const puLifeImg = new Image();
    puLifeImg.src = 'assets/powerup-life.png';
    const puRapidFireImg = new Image();
    puRapidFireImg.src = 'assets/stinger.png';

    // New Assets
    const droneImg = new Image();
    droneImg.src = 'assets/player.png';
    const enemyBulletImg = new Image();
    enemyBulletImg.src = 'assets/arrow.png';
    const bossHomingImg = new Image();
    bossHomingImg.src = 'assets/dreadstar.png';
    const bossClusterImg = new Image();
    bossClusterImg.src = 'assets/boomer.png';

    // New Boss Assets
    const ufo2Img = new Image(); ufo2Img.src = 'assets/minibos/ufo2.png';
    const ufo3Img = new Image(); ufo3Img.src = 'assets/minibos/ufo3.png';
    const ufo4Img = new Image(); ufo4Img.src = 'assets/minibos/ufo4.png';
    const ufo5Frames: HTMLImageElement[] = [];
    for (let i = 0; i < 25; i++) {
      const img = new Image();
      img.src = `assets/minibos/ufo5/green_ufo_gmc_${i.toString().padStart(4, '0')}.png`;
      ufo5Frames.push(img);
    }

    // Explosion Animation Assets
    const explosionFrames: HTMLImageElement[] = [];
    for (let i = 0; i < 12; i++) {
        const img = new Image();
        img.src = `assets/explotion_gif/explosion_${i}.png`; // Assuming this naming convention from user prompt
    ...
    let particles: any[] = [];
    let explosions: any[] = []; // For frame-based explosions
    let bossIndex = 1; // Boss Rotation Tracker (1-5)
    ...
    let nextBossSpawnTime = Math.floor(Math.random() * 900) + 1500; // 25-40 seconds (60fps)
    ...
    const update = () => {
      ...
      empShockwaves = empShockwaves.filter(sw => {
        sw.radius += 20;
        ...
        if (miniBoss) {
          const dx = sw.x - miniBoss.x;
          const dy = sw.y - miniBoss.y;
          const d = Math.sqrt(dx*dx + dy*dy);
          if (Math.abs(d - sw.radius) < 40) { miniBoss.stunTimer = STUN_DURATION; }
        }
      ...
      // --- MEMORY CLEANUP & PERFORMANCE OPTIMIZATION ---
      // We use .filter() to efficiently remove objects that are off-screen or destroyed.
      // This prevents memory leaks and game slowdowns over time.
      ...
      // Cleanup enemy bullets and handle complex patterns
      const newEnemyBulletsFromExplosions: any[] = [];
      enemyBullets = enemyBullets.filter(eb => {
        ...
        // Homing logic (Pattern C): Tracks player briefly
        if (eb.type === 'homing' && eb.homingTimer > 0) {
          eb.homingTimer--;
          const dx = player.x - eb.x;
          const dy = player.y - eb.y;
          const angle = Math.atan2(dy, dx);
          eb.vx += Math.cos(angle) * 0.5;
          eb.vy += Math.sin(angle) * 0.5;
          ...
          eb.x += eb.vx;
          eb.y += eb.vy;
        } else if (eb.type === 'cluster' && eb.clusterTimer > 0) {
          // Cluster Bomb logic (Pattern D): Moves to center then explodes
          eb.clusterTimer--;
          eb.x += (eb.targetX - eb.x) * 0.05;
          eb.y += (eb.targetY - eb.y) * 0.05;
          
          if (eb.clusterTimer <= 0) {
            // FIX: Spawn 8 red glowing projectiles symmetrically using separate collection array
            for (let k = 0; k < 8; k++) {
              const angle = k * (Math.PI / 4); // Symmetric 8-way spread
              const speed = 7;
              newEnemyBulletsFromExplosions.push({
                x: eb.x,
                y: eb.y,
                vx: Math.cos(angle) * speed,
                vy: Math.sin(angle) * speed,
                width: 14, // 14px diameter as requested (12-15 range)
                height: 14,
                isBoss: true,
                isClusterFragment: true, // Tag for special rendering
                stunTimer: 0
              });
            }
            createExplosion(eb.x, eb.y, '#ef4444'); // Red explosion effect
            playExplosionSound();
            return false;
          }
        } else if (eb.vx !== undefined) {
           eb.x += eb.vx;
           eb.y += eb.vy;
        } else {
           eb.x -= eb.speed;
        ...
        return eb.x >= -50 && eb.x <= width + 50 && eb.y >= -50 && eb.y <= height + 50;
      });
      // Merge new bullets into the main array
      if (newEnemyBulletsFromExplosions.length > 0) {
          enemyBullets = [...enemyBullets, ...newEnemyBulletsFromExplosions];
      }

      // --- MINI-BOSS SPAWN CYCLE ---
      // Bosses spawn in a fixed sequence (Index 1-5) every 30-60 seconds.
      // After Boss 5, it wraps back to Boss 1 with higher stats.
      if (bossSpawnTimer >= nextBossSpawnTime && !miniBoss && !warpActive && !levelEntryActive) {
        const patterns = ['laser', 'spread', 'homing', 'cluster'];
        ...
        miniBoss = {
          ...
          hp: (50 + bossCycleCount * 20), // Scalable HP per cycle
          maxHp: (50 + bossCycleCount * 20),
          speed: 1.5,
          shootTimer: 0,
          isCharging: false,
          chargeTimer: 0,
        ...
        bossSpawnTimer = 0;
        enemyTimer = -99999; // Pause standard spawning
      }
      ...
      if (!miniBoss) {
        ...
        // Boss Update
        if (miniBoss.stunTimer > 0) {
          miniBoss.stunTimer--;
        } else {
          miniBoss.x = Math.max(width - 250, miniBoss.x - miniBoss.speed);
          miniBoss.y += Math.sin(Date.now() / 1000) * 2;
          
          if (miniBoss.bossIndex === 5) {
            miniBoss.frame = (miniBoss.frame + 0.3) % 25;
          }

          miniBoss.shootTimer++;
          
          // Pattern Selection Logic: UFO 1-4 choose 2 random patterns per cycle, UFO 5 uses all 4.
          const attackCycle = 180; // 3 second cycle
          const localTime = miniBoss.shootTimer % attackCycle;
          
          if (localTime === 0) {
              const allPatterns = ['laser', 'spread', 'homing', 'cluster'];
              if (miniBoss.bossIndex < 5) {
                  // Pick 2 Unique Random Patterns
                  const shuf = [...allPatterns].sort(() => Math.random() - 0.5);
                  miniBoss.activePatterns = [shuf[0], shuf[1]];
              } else {
                  miniBoss.activePatterns = allPatterns;
              }
          }

          if (miniBoss.activePatterns) {
              miniBoss.activePatterns.forEach((p: string) => {
                  if (p === 'laser') {
                      if (localTime < 90 && miniBoss.shootTimer % 10 === 0) {
                          enemyBullets.push({
                              x: miniBoss.x - 80,
                              y: miniBoss.y,
                              vx: -15,
      ...
      powerUps = powerUps.filter(pu => {
        ...
        if (isColliding(player, pu)) {
          ...
          if (pu.type === 'shield') {
            ...
            buffTimers.shield = 420;
          } else if (pu.type === 'gun') {
            const pick = Math.random() > 0.5 ? 'rapidFire' : 'doubleShot';
            (buffTimers as any)[pick] = 420;
            if (pick === 'rapidFire') player.shootingSpeed = BUFFED_SHOOTING_SPEED;
          } else if (pu.type === 'rapidfire') {
            buffTimers.rapidFire = 600; // 10 seconds
            player.shootingSpeed = BUFFED_SHOOTING_SPEED;
      ...
      bullets = bullets.filter(bullet => {
        ...
        if (miniBoss && isColliding(bullet, miniBoss)) {
          ...
          if (miniBoss.hp <= 0) {
            playExplosionSound();
            explosions.push({ x: miniBoss.x, y: miniBoss.y, frame: 0, scale: 3 });
            currentScore += 100 * combo;
            
            // Advance Boss Tracker
            bossIndex++;
            if (bossIndex > 5) {
              bossIndex = 1;
              bossCycleCount++;
        ...
        if (active) {
          enemies.forEach((enemy, i) => {
            if (active && isColliding(bullet, enemy)) {
              active = false;
              enemy.hp--;
              createExplosion(bullet.x, bullet.y, '#ffaa00');
              if (enemy.hp <= 0) {
                playExplosionSound();
                explosions.push({ x: enemy.x, y: enemy.y, frame: 0, scale: 1 });
                // Overdrive meter increment
                if (!overdrive.active) {
                ...
                // Power-up Drop Chance (15%): Shield, Gun, Life, RapidFire
                if (Math.random() < 0.15) {
                  const types = ['shield', 'gun', 'life', 'rapidfire'];
      ...
      // Damage values based on requested requirements:
      // - Normal Shot: 10%
      // - Boss Shot: 25%
      // - Normal Body: 20%
      // - Boss Body: 60%
      // - Energy Shield (ShieldHP) always takes damage first with carryover.
      const handlePlayerHit = (damage: number) => {
        ...
        // Tracking for shield regeneration delay
        player.lastDamageTime = Date.now();

        // 1. Process Energy Shield (Refilled via Power-ups or Bounty)
        if (player.shieldHp > 0) {
          player.shieldHp -= damage;
          if (player.shieldHp < 0) {
            // Subtract remainder from core health
            player.hp += player.shieldHp; 
            player.shieldHp = 0;
          }
          screenShake = 5;
          playPowerupSound();
        ...
        // 2. Process Temporary Power-up Shield (Invulnerability layer)
      ...
      enemies.forEach(enemy => {
        if (isColliding(player, enemy) && !isDying) handlePlayerHit(20); // 20% Damage for colliding with normal enemy
      });

      enemyBullets.forEach((eb, i) => {
        if (isColliding(player, eb)) {
          enemyBullets.splice(i, 1);
          // 10% for normal, 25% for boss
          handlePlayerHit(eb.isBoss ? 25 : 10);
        }
      });

      if (miniBoss && isColliding(player, miniBoss) && !isDying) handlePlayerHit(60); // 60% Damage for colliding with Mini-Boss
      ...
      // Plays at ~0.3 speed to ensure smooth visualization of all 12 frames (approx 0.6 seconds at 60fps)
      explosions = explosions.filter(exp => {
        exp.frame += 0.3; 
        return Math.floor(exp.frame) < 12;
      });
      ...
      // 1.2 Emergency Repair Drone Logic (Triggered by 10 Scraps)
      if (repairDrone.active) {
        ...
        // Drone shooting assistance
        if (repairDrone.timer % 30 === 0) {
            bullets.push({ 
                x: player.x, 
                y: player.y - 30, // Drone position offset
                width: 10, 
                height: 3, 
                speed: 15,
                isDroneBullet: true 
        ...
        if (repairDrone.timer <= 0) {
            repairDrone.active = false;
            scrapCount = 0; // Reset scrap count after duration
        }
      }
      ...
      scraps.forEach((s, i) => {
        ...
        if (isColliding(player, s)) {
          scraps.splice(i, 1);
          scrapCount++;
          if (scrapCount >= 10 && !repairDrone.active) {
            repairDrone.active = true;
            repairDrone.timer = 600; // 10 seconds (600 frames)
            repairDrone.healTick = 0;
            playPowerupSound();
          }
          playPowerupSound();
    ...
    const draw = () => {
      ...
      // Draw Explosions (Frame-based from assets/explotion_gif)
      explosions.forEach(exp => {
        ...
        if (img && img.complete && img.naturalWidth !== 0) {
            const size = 64 * exp.scale;
            ctx.drawImage(img, -size / 2, -size / 2, size, size);
        } else {
            // Fallback to programmatic effect
            const progress = exp.frame / 12;
            const outerRadius = 40 * exp.scale * progress;
            const innerRadius = 20 * exp.scale * progress;
      ...
      enemyBullets.forEach((eb) => {
        if (eb.isClusterFragment) {
            // Special Rendering for Cluster Fragments: Neon Red Glowing Core
            ctx.save();
            ctx.translate(eb.x, eb.y);
            ctx.shadowBlur = 12;
            ctx.shadowColor = 'red';
            ctx.fillStyle = '#ff0000'; // Bright neon red
            ctx.beginPath();
            ctx.arc(0, 0, eb.width / 2, 0, Math.PI * 2);
            ctx.fill();
            
            // Inner core glow to enhance neon effect
            ctx.fillStyle = 'white';
            ctx.globalAlpha = 0.7;
            ctx.beginPath();
            ctx.arc(0, 0, eb.width / 3, 0, Math.PI * 2);
            ctx.fill();
            ctx.restore();
            return;
        }

        if (eb.type === 'homing' && bossHomingImg.complete && bossHomingImg.naturalWidth !== 0) {
            // Visual Baru Pola Serangan C (Homing Missile)
            ctx.save();
            ctx.translate(eb.x, eb.y);
            ctx.rotate(Math.atan2(eb.vy, eb.vx));
            ctx.shadowBlur = 15;
            ctx.shadowColor = 'cyan';
            const w = eb.width * 1.5;
            const h = eb.height * 1.5;
            ctx.drawImage(bossHomingImg, -w / 2, -h / 2, w, h);
            ctx.restore();
            return;
        }

        if (eb.type === 'cluster' && eb.clusterTimer > 0 && bossClusterImg.complete && bossClusterImg.naturalWidth !== 0) {
            // Visual Baru Pola Serangan D (Cluster Bomb) - Only while flying to center
            ctx.save();
            ctx.translate(eb.x, eb.y);
            ctx.shadowBlur = 15;
            ctx.shadowColor = 'orange';
            const w = eb.width * 1.2;
            const h = eb.height * 1.2;
            ctx.drawImage(bossClusterImg, -w / 2, -h / 2, w, h);
            ctx.restore();
            return;
        }
        ...
        if (enemyBulletImg.complete && enemyBulletImg.naturalWidth !== 0) {
          ...
          // Enhanced glow for enemy bullets
          ctx.shadowBlur = 15;
          ctx.shadowColor = eb.isBoss ? '#ff00ff' : '#ffaa00';
          
          if (eb.vx !== undefined && eb.vy !== undefined) {
             ctx.rotate(Math.atan2(eb.vy, eb.vx));
          } else {
             ctx.rotate(Math.PI);
          }
          
          // Increased size 1.3x as requested
          const w = eb.width * 1.3;
          const h = eb.height * 1.3;
          ctx.drawImage(enemyBulletImg, -w / 2, -h / 2, w, h);
          ctx.restore();
        } else {
      ...
      // Draw Scraps (White glowing circle)
      scraps.forEach(s => {
        ctx.save();
      ...
      if (miniBoss) {
        ...
        let bImg = alienImg;
        if (miniBoss.bossIndex === 2) bImg = ufo2Img;
        else if (miniBoss.bossIndex === 3) bImg = ufo3Img;
        else if (miniBoss.bossIndex === 4) bImg = ufo4Img;
        else if (miniBoss.bossIndex === 5) bImg = ufo5Frames[Math.floor(miniBoss.frame)];

        if (bImg.complete && (bImg as any).naturalWidth !== 0) {
          ctx.drawImage(bImg, -miniBoss.width / 2, -miniBoss.height / 2, miniBoss.width, miniBoss.height);
        } else {
      ...
      powerUps.forEach((pu) => {
        ...
        let img = puShieldImg;
        if (pu.type === 'gun') img = puGunImg;
        else if (pu.type === 'life') img = puLifeImg;
        else if (pu.type === 'rapidfire') img = puRapidFireImg;

        if (img.complete && img.naturalWidth !== 0) {
          const w = pu.width * pulseScale;
          const h = pu.height * pulseScale;
          ...
          ctx.beginPath();
          ctx.arc(0, 0, (pu.width / 2) * pulseScale, 0, Math.PI * 2);
          ctx.fill();
        }
        ctx.restore();
      });
